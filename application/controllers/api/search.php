...
class Search extends Api_controller {
  ...
  public function index () {
    ...
    $conditions = $next_id ? array ('id <= ? AND is_enabled = ? AND (title LIKE ? OR tags LIKE ? OR menu_id IN (?))', $next_id, Article::ENABLE_YES, '%' . $keywords . '%', '%' . $keywords . '%', $menu_ids ? $menu_ids : array (0)) : array ('is_enabled = ? AND (title LIKE ? OR tags LIKE ? OR menu_id IN (?))', Article::ENABLE_YES, '%' . $keywords . '%', '%' . $keywords . '%', $menu_ids ? $menu_ids : array (0));
